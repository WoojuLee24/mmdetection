...
temper_ratio test
"""
def supcontrast_maskv0_01_tr(logits_anchor, logits_contrast, targets, mask_anchor, mask_contrast, lambda_weight=0.1, temper=0.07, temper_ratio=1.):
...
def supcontrastv1_3(logits_clean, labels=None, lambda_weight=0.1, temper=0.07, min_samples=10,
                            reduction='batchmean', **kwargs):
    """
        supcontrast loss
        create mask anchor and masks contrast
    ...
    if inds.size(0) > min_samples:
        ...
        mask_instance = torch.zeros(batch_size, batch_size, dtype=torch.float32).to(device)
        mask_eye_half = torch.eye(batch_size_half, dtype=torch.float32).to(device)
        mask_instance[batch_size_half:, :batch_size_half] += mask_eye_half
        mask_instance[:batch_size_half, batch_size_half:] += mask_eye_half
        # mask_instance_np = mask_instance.detach().cpu().numpy()
        mask_instance_bg = mask_instance * mask_bg
        # mask_instance_bg_np = mask_instance_bg.detach().cpu().numpy()

        mask_anchor_fg = mask_anchor_except_eye * mask_fg
        mask_anchor = mask_anchor_fg + mask_instance_bg
...
def supcontrast_clean_fg(logits_clean, labels=None, lambda_weight=0.1, temper=0.07, min_samples=10, reduction='batchmean'):
...
def supcontrast_clean_fg_bg_old(logits_clean, labels=None, lambda_weight=0.1, temper=0.07, min_samples=10, reduction='batchmean', **kwargs):
    ...
    if inds.size(0) > min_samples:
        ...
        mask_contrast_except_eye = mask_contrast - mask_eye
        mask_contrast_except_eye = mask_contrast_except_eye.detach()
        # mask_contrast_exept_eye_np = mask_contrast_except_eye.detach().cpu().numpy()
...
def supcontrast_clean_fg_bg(logits_clean, labels=None, lambda_weight=0.1, temper=0.07, min_samples=10, reduction='batchmean', **kwargs):
    ...
    """
        ...
        mask_anchor_except_eye: same target except self-case
        mask_anchor_fg: same fg target except self-case
        mask_contrast_except_eye: same target except self-case (same as mask_anchor_except_eye)
    ...
    if inds.size(0) > min_samples:
        ...
        mask_contrast = torch.ones([batch_size, batch_size], dtype=torch.float32).to(device)
        # mask_contrast_except_eye = mask_contrast - mask_eye
        mask_contrast_except_eye_ori = mask_contrast - mask_eye
        mask_contrast_except_eye_ori_np = mask_contrast_except_eye_ori.detach().cpu().numpy()
        mask_contrast_except_eye = mask_anchor_except_eye.detach()
        mask_contrast_exept_eye_np = mask_contrast_except_eye.detach().cpu().numpy()
...
def supcontrastv0_2_0(logits_clean, labels=None, lambda_weight=0.1, temper=0.07, min_samples=10,
                    reduction='batchmean', fg_iou=None, iou_act='x', iou_th=0.5, **kwargs):
    ...
    if inds.size(0) > min_samples:
        ...
        # iou mask
        ious = torch.zeros_like(targets, dtype=torch.float32)
        assert inds.size(0) == fg_iou.size(0)
        ious[inds, 0] = fg_iou
        mask_iou = torch.matmul(ious, ious.T)
        # mask_iou_np = mask_iou.cpu().detach().numpy()
        if iou_act == 'x':
            mask_iou = torch.where(mask_iou > iou_th, mask_iou, torch.tensor(0, device=device, dtype=torch.float32))
        else:
            mask_iou = torch.where(mask_iou > iou_th,
                                   torch.tensor(1, device=device, dtype=torch.float32),
                                   torch.tensor(0, device=device, dtype=torch.float32))
        # mask_iou_np2 = mask_iou.cpu().detach().numpy()
        ...
        mask_anchor = torch.eq(targets, targets.T).float()  # [B, B]
        mask_anchor_except_eye = mask_anchor - mask_eye
        mask_anchor_except_eye = mask_anchor_except_eye * mask_iou
        # mask_anchor_except_eye_np = mask_anchor_except_eye.detach().cpu().numpy()
...
def supcontrastv0_2(logits_clean, labels=None, lambda_weight=0.1, temper=0.07, min_samples=10,
                    reduction='batchmean', fg_iou=None, iou_act='x', iou_th=0.5, **kwargs):
...
def supcontrast_clean_fg_bg_tr(logits_clean, labels=None, lambda_weight=0.1, temper=0.07, temper_ratio=1., min_samples=10, reduction='batchmean'):
    ...
    if inds.size(0) > min_samples:
        ...
        loss1 = supcontrast_maskv0_01_tr(logits_clean, logits_clean, targets,
                                         mask_anchor_except_eye, mask_contrast_except_eye,
