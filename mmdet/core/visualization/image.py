...
def imshow_det_bboxes_filter(img,
                             bboxes,
                             labels,
                             gt_bboxes,
                             gt_labels,
    ...
    for i, (bbox, label) in enumerate(zip(bboxes, labels)):
        ...
        # Score
        bboxes1 = torch.Tensor([bbox[:-1]]).unsqueeze(0)
        ious = bbox_overlaps(bboxes1, gt_bboxes, is_aligned=False)
        iou = ious[0,0,:]
        valid_iou = iou[iou > valid_iou_threshold]
        bbox_color = (0, 255, 0)
        # if len(valid_iou) == 0:
        #     bbox_color = (0, 0, 255)
        # else:
