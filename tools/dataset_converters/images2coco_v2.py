...
def collect_image_infos2(path, exclude_extensions=None):
    img_infos = []

    scenario_list = sorted(os.listdir(path))
    for scenario in scenario_list:
        scenario_path = os.path.join(path, scenario)
        image_list = sorted(os.listdir(scenario_path))
        for image in image_list:
            if exclude_extensions is None or (
                    exclude_extensions is not None
                    and not image_path.lower().endswith(exclude_extensions)):
                # image_path = os.path.join(path, image_path)
                image_path = os.path.join(scenario_path, image)
                image_path_json = os.path.join(scenario, image)
                img_pillow = Image.open(image_path)
                img_info = {
                    'filename': image_path_json,
                    'width': img_pillow.width,
                    'height': img_pillow.height,
                }
                img_infos.append(img_info)
...
def main():
    ...
    img_infos = collect_image_infos2(args.img_path, args.exclude_extensions)
