...
def collect_image_infos(path, exclude_extensions=None):
    ...
    for image_path in mmcv.track_iter_progress(sorted(list(images_generator))):
        if exclude_extensions is None or (
                exclude_extensions is not None
                and not image_path.lower().endswith(exclude_extensions)):
            image_abs_path = os.path.join(path, image_path)
            img_pillow = Image.open(image_abs_path)
            img_info = {
                'filename': image_path,
