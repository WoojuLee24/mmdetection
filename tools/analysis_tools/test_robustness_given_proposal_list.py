    ...
    for corr_i, corruption in enumerate(corruptions):
        ...
        for sev_i, corruption_severity in enumerate(args.severities):
            ...
            if not distributed:
                ...
                debug_cfg = dict(out_dir=show_dir,
                                 corruption=corruption,
                                 corruption_severity=corruption_severity,
                                 given_proposal_list=True,
                                 # save_list=['original_image',
                                 #            'proposal_list_score_distribution',
                                 #            'proposal_list_score_density',
                                 #            'bbox_results_score_distribution',
                                 #            'bbox_results_score_density']
                                 save_list = []
                                 )
                outputs = single_gpu_test(model, data_loader, args.show,
                                          show_dir, args.show_score_thr, debug_cfg=debug_cfg)
            else:
