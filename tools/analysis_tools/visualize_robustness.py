...
from mmdet.apis import multi_gpu_test, set_random_seed, single_gpu_visualize
...
def parse_args():
    ...
    parser.add_argument('--gt', action='store_true', help='gt')
    parser.add_argument(
...
def main():
    ...
    for corr_i, corruption in enumerate(corruptions):
        ...
        for sev_i, corruption_severity in enumerate(args.severities):
            ...
            # test_data_cfg['ann_file'] = '/ws/data/cityscapes/annotations/vis_mini.json'
            test_data_cfg['ann_file'] = '/ws/data/cityscapes/annotations/paper_vis.json'
            test_data_cfg['test_mode'] = False
            test_data_cfg['pipeline'][-1]['transforms'][-1]['keys'].append('gt_bboxes')
            test_data_cfg['pipeline'][-1]['transforms'][-1]['keys'].append('gt_labels')
            test_data_cfg['pipeline'].insert(1, dict(type='LoadAnnotations', with_bbox=True))
            ...
            if not distributed:
                ...
                outputs = single_gpu_visualize(model, data_loader, args.show,
                                               show_dir, args.show_score_thr, args.gt)
            else:
                raise NotImplementedError
