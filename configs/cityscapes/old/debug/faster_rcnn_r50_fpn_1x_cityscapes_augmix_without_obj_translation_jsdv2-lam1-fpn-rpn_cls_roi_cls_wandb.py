...
model = dict(
    rpn_head=dict(
        ...
        loss_bbox=dict(type='L1LossPlus', loss_weight=1.0,
                       additional_loss='None', lambda_weight=1, temper=1, wandb_name='rpn_bbox')),
    roi_head=dict(
        bbox_head=dict(
            loss_cls=dict(
            ...
            loss_bbox=dict(type='SmoothL1LossPlus', beta=1.0, loss_weight=1.0,
                           additional_loss='None', lambda_weight=1, temper=1, wandb_name='roi_bbox'))),
    train_cfg=dict(
        wandb=dict(
            log=dict(
                features_list=['neck.fpn_convs.0.conv', 'neck.fpn_convs.1.conv'],
                vars=['log_vars'],
                ))))
