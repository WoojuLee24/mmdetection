...
model = dict(
    rpn_head=dict(
        loss_cls=dict(
            ...
            , additional_loss='none', lambda_weight=12, wandb_name='rpn_cls'),
        loss_bbox=dict(type='L1LossPlus', loss_weight=1.0
                       , additional_loss="None", lambda_weight=0.0001, wandb_name='rpn_bbox'),
        loss_additional=dict(type='RpnAdditionalLoss', version='2.4', weight=12, wandb_name='additional_loss(rpn_cls)')),
    roi_head=dict(
        bbox_head=dict(
            loss_cls=dict(
    ...
    train_cfg=dict(
        wandb=dict(
            log=dict(
                features_list=[],
                vars=['log_vars']),),
        given_cfg=dict(type='proposal_list', parent_dir='/ws/data/ai28/debug/given_resources/proposal_list_mini/',
                       train_dataset_img_prefix='/ws/data/cityscapes-mini/val/')
    ))
...
data = dict(
    ...
    train=dict(
        dataset=dict(
            ann_file='/ws/data/cityscapes/annotations/instancesonly_filtered_gtFine_mini.json',
...
if str_loss == 'plus':
    ...
    if 'tail_version' in rpn_loss_cls:
        str_each_loss += rpn_loss_cls['tail_version']
...
if 'lambda_weight' in rpn_loss_cls:
    lw = rpn_loss_cls['lambda_weight']
elif 'lambda_weight' in rpn_loss_bbox:
    lw = rpn_loss_bbox['lambda_weight']
elif 'lambda_weight' in roi_loss_cls:
    lw = roi_loss_cls['lambda_weight']
elif 'lambda_weight' in roi_loss_bbox:
    lw = roi_loss_bbox['lambda_weight']
if lw is not -999:
    str_parameters += ('_lw.'+("{:.0e}".format(lw) if lw < 0 else "{:d}".format(lw)))
print('++++++++++++++++++++')
