...
model = dict(
    rpn_head=dict(
        loss_cls=dict(
            ...
            additional_kwargs=dict(lambda_weight=0.001, avg_factor=None, temper=1)
        ),
    ...
    roi_head=dict(
        bbox_head=dict(
            loss_cls=dict(
                ...
                , additional_loss='None', wandb_name='roi_cls'),
            loss_bbox=dict(type='SmoothL1LossPlus', beta=1.0, loss_weight=1.0
                           , additional_loss="None", wandb_name='roi_bbox'))),
    train_cfg=dict(
        wandb=dict(
            log=dict(
                features_list=[],
...
for loss in losses:
    if 'additional_kwargs' in loss:
        for k, v in loss['additional_kwargs'].items():
            if k is 'lambda_weight':
                str_parameters += f"_lw."+"{:.0e}".format(v)
            else:
                k = k.replace('_', '')
                str_parameters += f"_{k}.{v}"
