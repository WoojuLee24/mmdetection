...
model = dict(
    rpn_head=dict(
        loss_cls=dict(
            ...
            , additional_loss='rpn_tail', lambda_weight=12, wandb_name='rpn_cls'
            , tail_version='v1.6'
            , channel_wise='avgpool'
            , maxpool_criterion=dict(type='v1.1', kernel_size=2)
            , interpolate=dict(type='v1.1', thr_h=50, sizes=[[20,40], [10,20]])
            , linear_criterion=dict(type='v1.1', out_channels=[10*20, 5*10])),
