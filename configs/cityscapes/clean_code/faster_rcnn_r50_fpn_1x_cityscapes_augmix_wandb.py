...
[OPTIONS]
  ...
  * train_cfg.wandb.log.features_list 
    : ["rpn_head.rpn_cls", "neck.fpn_convs.0.conv", "neck.fpn_convs.1.conv", "neck.fpn_convs.2.conv", "neck.fpn_convs.3.conv"] 
'''
...
model = dict(
    rpn_head=dict(
        ...
        loss_bbox=dict(type='L1LossPlus', loss_weight=1.0
                       , additional_loss='jsdy', lambda_weight=0.0001, wandb_name='rpn_bbox')),
    roi_head=dict(
        bbox_head=dict(
            loss_cls=dict(
            ...
            loss_bbox=dict(type='SmoothL1LossPlus', beta=1.0, loss_weight=1.0
                           , additional_loss='jsdy', lambda_weight=0.0001, wandb_name='roi_bbox'))),
    train_cfg=dict(
        wandb=dict(
            log=dict(
                  ...
                  hooks=[
                      ...
                      dict(type='WandbLogger',
                           wandb_init_kwargs={'project': "AI28", 'entity': "ai28",
                                              'name': f"AugMix--RPN.{rpn_loss_cls['additional_loss']}.{rpn_loss_bbox['additional_loss']}__"
