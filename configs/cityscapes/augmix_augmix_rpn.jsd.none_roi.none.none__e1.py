...
model = dict(
    rpn_head=dict(
        loss_cls=dict(type='CrossEntropyLossAugMix', use_sigmoid=True, loss_weight=1.0),
...
if pipeline is not None:
    ...
    str_pipeline += f".{pipeline['aug_list'].lower()}" if hasattr(pipeline, 'aug_list') else ''
else:
...
if str_loss == 'plus':
    str_each_loss = "rpn"
    str_each_loss += f".{rpn_loss_cls['additional_loss'] if hasattr(rpn_loss_cls, 'additional_loss') else 'none'}"
    str_each_loss += f".{rpn_loss_bbox['additional_loss'] if hasattr(rpn_loss_bbox, 'additional_loss') else 'none'}"
    str_each_loss += '_roi'
    str_each_loss += f".{roi_loss_cls['additional_loss'] if hasattr(roi_loss_cls, 'additional_loss') else 'none'}"
    str_each_loss += f".{roi_loss_bbox['additional_loss'] if hasattr(roi_loss_bbox, 'additional_loss') else 'none'}"
...
if hasattr(rpn_loss_cls, 'lambda_weight'):
    str_parameters += ('_'+str(rpn_loss_cls['lambda_weight']))
elif hasattr(rpn_loss_bbox, 'lambda_weight'):
    str_parameters += ('_'+str(rpn_loss_bbox['lambda_weight']))
elif hasattr(roi_loss_cls, 'lambda_weight'):
    str_parameters += ('_'+str(roi_loss_cls['lambda_weight']))
elif hasattr(roi_loss_bbox, 'lambda_weight'):
    str_parameters += ('_'+str(roi_loss_bbox['lambda_weight']))
if pipeline is not None:
    str_parameters += ('_'+pipeline['lambda_weight']) if hasattr(pipeline, 'lambda_weight') else ''
