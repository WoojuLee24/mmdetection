...
model = dict(
    rpn_head=dict(
        loss_cls=dict(
            ...
            , additional_loss='jsdv2', lambda_weight=0.1, temper=1, wandb_name='rpn_cls'),
    ...
    roi_head=dict(
        bbox_head=dict(
            loss_cls=dict(
                ...
                , additional_loss='jsdv2', lambda_weight=0.0001, wandb_name='roi_cls'),
            # loss_bbox=dict(type='SmoothL1LossPlus', beta=1.0, temper=1, loss_weight=1.0
...
if ('lambda_weight' in rpn_loss_cls) or ('lambda_weight' in roi_loss_cls)\
        or ('lambda_weight' in rpn_loss_bbox) or ('lambda_weight' in roi_loss_bbox):
    str_parameters += '_lw'
    if 'lambda_weight' in rpn_loss_cls:
        str_parameters += ('.' + "{:.0e}".format(rpn_loss_cls['lambda_weight']))
    if 'lambda_weight' in rpn_loss_bbox:
        str_parameters += ('.' + "{:.0e}".format(rpn_loss_bbox['lambda_weight']))
    if 'lambda_weight' in roi_loss_cls:
        str_parameters += ('.' + "{:.0e}".format(roi_loss_cls['lambda_weight']))
    if 'lambda_weight' in roi_loss_bbox:
        str_parameters += ('.'+"{:.0e}".format(roi_loss_bbox['lambda_weight']))
