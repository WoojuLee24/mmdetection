    ...
    roi_head=dict(
        bbox_head=dict(
            loss_cls=dict(
                ...
                , additional_loss='jsdv1_3', lambda_weight=1, wandb_name='roi_cls', log_pos_ratio=True),
            loss_bbox=dict(type='SmoothL1LossPlus', beta=1.0, loss_weight=1.0
                           , additional_loss="l1_margin_lossv0_1", margin=0.01, lambda_weight=1, wandb_name='roi_bbox'))),
    train_cfg=dict(
        wandb=dict(
            log=dict(
                features_list=[],
...
train_pipeline = [
    ...
    ### AugMix ###
    dict(type='AugMixDetection', num_views=3, version='1.3',
...
roi_loss = 'jsdv1.3.l1marginv0.1'
lambda_weight = '1e-1.1.1'
...
print(f"{name}")
print('++++++++++++++++++++')
# model['train_cfg'].update({'log_loss_region': f'/ws/data/dshong/ai28_detection/analysis_loss_region/{name}'})
