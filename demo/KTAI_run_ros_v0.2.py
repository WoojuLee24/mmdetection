...
# pubFeat = rospy.Publisher('/deep/feat', Float32MultiArray, queue_size=10)
...
def parse_args():
    ...
    parser.add_argument('--checkpoint', default='../configs/aioneteam/faster_rcnn_r50_fpn_1x_coco_20200130-047c8118.pth', help = 'Checkpoint file')
...
def callback_image(data: Image):
    ...
    # evalimage(img).astype(np.uint8)
    # seg_cv
    img_seg = bridge.cv2_to_imgmsg(seg_cv)

    # pubSeg.publish(img_seg)

    # bbMsg = Float32MultiArray()
    # bbMsg.data = [[1.2354567, 99.7890, 67.654236], [67.875, 90.6543, 76.5689], [65.3452, 45.873, 67.8956]]
    #
    # pubBB.publish(bbMsg)
    #
    # featMsg = Float32MultiArray()
    # featMsg.data = [[1.2354567, 99.7890, 67.654236], [67.875, 90.6543, 76.5689], [65.3452, 45.873, 67.8956]]
    # pubFeat.publish(featMsg)
...
if __name__ == '__main__':
    try:
        ...
        rospy.Subscriber("/d400/color/image_raw", Image, callback_image, queue_size=100000)
        rospy.spin()
